...
import static cn.chengzhiya.mhdfbot.util.Util.getMemberList;
        ...
        if (Objects.requireNonNull(Util.getConfig().getStringList("AllowUseBotList")).contains(event.getGroupId().toString()) && event.getMessage().startsWith("#")) {
            ...
            switch (args[0]) {
                ...
                case "#绑定":
                    ...
                    if (bot.getStrangerInfo(event.getUserId(), false).getData().getLevel() >= Util.getConfig().getInt("BindSettings.MinQQLevel")) {
